import React from "react";
import { Button } from "./Button";

type HeroProps = {};

const Hero = (props: HeroProps) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2  p-2   mx-auto  max-w-7xl">
      <div className="flex flex-col gap-9  ">
        <h1 className=" text-6xl mt-12  font-semibold text-salesforce-950">
          Try Salesforce <br /> Starter Suite for <br /> free.
        </h1>
        <p className="   font-light text-lg">
          Unite marketing, sales, and service in a single app. Try <br />
          Salesforce Starter Suite today. There's nothing to install. <br /> No
          credit card required.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:w-3/5">
          <Button> Start free triel</Button>
          <Button variant="secondary"> Wetch demos</Button>
        </div>
      </div>
    </div>
  );
};

export { Hero };
